...
playerObj.draw(2, goToFirstHand(playerCardMap), cards, deckLength(cards));
dealerObj.draw(2, goToFirstHand(dealerCardMap), cards, deckLength(cards));

//dealerObj.testDraw(2, goToFirstHand(dealerCardMap), cards, cards.size, 10, 10);
//playerObj.testDraw(2, goToFirstHand(playerCardMap), cards, cards.size, 13, 13);

const reset = () => 
{
        dealerObj.deleteHand(goToFirstHand(dealerCardMap));
        dealerObj.resetHand(cards, deckLength(cards));
        dealerObj.clearValueArr();
        playerObj.resetHand(cards, deckLength(cards));
...
const stand = (modifiedBet) =>
{
    ...
    if (playerCardMap.size > 0)
            {
                //temp = playerObj.printHand(goToFirstHand(playerCardMap));
            ...
            {
                dealerObj.dealerPlays(goToFirstHand(dealerCardMap), cards, deckLength(cards));
            }
            console.log("Dealer: ");
...
while (gamePrompt != "quit")
    {
        ...
        if (gamePrompt == "h") 
            {
                playerObj.hit(goToFirstHand(playerCardMap), cards, deckLength(cards));
        ...
        if (gamePrompt == "sp")
        {
            playerObj.split(goToFirstHand(playerCardMap), cards, deckLength(cards));
        };
        ...
        if (gamePrompt === "d")
        {
            let doubledBet = playerObj.doubleDown(goToFirstHand(playerCardMap), bet, cards, deckLength(cards));
